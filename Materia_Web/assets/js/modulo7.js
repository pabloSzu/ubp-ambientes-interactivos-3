...
const ARCH_SCENARIOS = [
  {
    ...
    options: ['CPU cluster de 64 cores de servidor', 'GPU NVIDIA RTX 4090 (16K CUDA cores)', 'FPGA + CPU híbrido', 'Cloud CPU burst (AWS c7g)'],
    correct: 1,
    feedback: '✅ GPU es la respuesta correcta. Ray tracing es el caso ideal para GPU: millones de operaciones de trazado completamente independientes (embarassingly parallel). La RTX 4090 tiene hardware específico para ray tracing (RT Cores). Un CPU haría el trabajo 100× más lento.',
  },
  {
    icon: '🌡️',
    title: 'Simulación meteorológica global',
    desc: 'El servicio meteorológico necesita simular el clima global para los próximos 10 días. La grilla tiene 100M de celdas 3D con ecuaciones de Navier-Stokes y fuertes dependencias entre celdas vecinas. Resultado en < 2 horas.',
    reqs: ['100M celdas 3D', 'Dependencias entre vecinos', 'Ecuaciones diferenciales', '< 2 horas'],
    options: ['GPU farm de 100× A100', 'Supercomputador HPC con MPI (miles de CPUs)', 'Cloud GPU (AWS p5)', 'ASIC diseñado específicamente'],
    correct: 1,
    feedback: '✅ HPC con MPI es correcto. Las simulaciones CFD tienen dependencias fuertes entre celdas vecinas — no son embarassingly parallel. Necesitan comunicación frecuente entre nodos via MPI. Los supercomputadores como Frontier usan miles de CPUs con InfiniBand para esta clase de workload.',
  },
  {
    icon: '📈',
    title: 'Trading de alta frecuencia (HFT)',
    desc: 'Decisiones de compra/venta en menos de 10 microsegundos. El código evalúa condiciones complejas con mucho branching (if/else) y envía órdenes al mercado. La latencia es más crítica que el throughput.',
    reqs: ['Latencia < 10 µs', 'Mucho branching', 'Decisiones complejas', 'Latencia > throughput'],
    options: ['GPU cluster para procesar en paralelo', 'Cloud con auto-scaling horizontal', 'CPU single-thread 5GHz+ + FPGA front-end', 'TPU de Google para ML'],
    correct: 2,
    feedback: '✅ CPU high-clock + FPGA es correcto. HFT es donde la GPU pierde: sufre warp divergence con el branching y tiene overhead de transferencia CPU→GPU. Los HFT usan CPUs Intel de 5GHz+, código C++ sin GC, kernel bypass DPDK, y FPGAs para el front-end de red a 40MHz.',
  },
  {
    icon: '🤖',
    title: 'Entrenamiento de LLM de 70B parámetros',
    desc: 'Tu empresa quiere entrenar un modelo de lenguaje de 70 billones de parámetros. El modelo no cabe en una sola GPU (requiere 140GB+ de VRAM para entrenamiento). El proceso tomará semanas y el presupuesto importa.',
    reqs: ['140GB+ VRAM', 'Semanas de entrenamiento', 'No cabe en 1 GPU', 'Costo importa'],
    options: ['Una sola GPU H100 de 80GB', 'CPU cluster con mucha RAM (1TB+)', 'Multi-GPU con NVLink + Cloud Spot instances', 'FPGA farm customizado'],
    correct: 2,
    feedback: '✅ Multi-GPU + Cloud Spot es correcto. Un modelo de 70B no cabe en una sola H100 (80GB VRAM). La solución es model parallelism: partir el modelo en múltiples GPUs conectadas con NVLink (900 GB/s). Para reducir el costo de semanas de cómputo, Spot instances con checkpointing frecuente son la solución estándar en la industria.',
  },
];
